...
const role = () => {
  ...
  const onPermissionSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_ENDPOINT}/manage/role/permissions`,
        {
  ...
  const onRoleSubmit = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_ENDPOINT}/manage/role/create`,
        {
  ...
  const onRoleDelete = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_ENDPOINT}/manage/role/delete`,
        {
      ...
      if (response.data.success == true) {
        // const response2 = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/manage/role/roles");
